...
- **Flujos alternativos:**
  - Un jugador **activo** sin rendimiento registrado esa semana: su cotización igualmente se recalcula en ese ciclo — nunca queda congelada al valor de la semana anterior ni se salta el cálculo para ese jugador. **Nota para diseño técnico (`tecnico`):** el criterio concreto de cómo se trata la ausencia de datos de partido dentro del algoritmo (por ejemplo, tratamiento neutro, decaimiento u otro criterio) es una decisión técnica pendiente de definición en esa etapa, no una decisión de producto.
  - Un jugador dado de baja (inactivo, ver UC-15) queda **excluido** del recálculo semanal: su cotización permanece congelada en el valor del día anterior a la baja, el mismo que se usó para compensar a los usuarios afectados. No se genera una nueva cotización semanal para él.
- **Criterios de aceptación:**
  - Dado que se cumple el ciclo semanal, cuando el sistema ejecuta el recálculo, entonces todos los jugadores **activos** del catálogo (incluidos los que no tuvieron rendimiento registrado esa semana) tienen una nueva cotización vigente, y la cotización anterior queda accesible en el historial.
  - Un jugador dado de baja no participa del recálculo semanal: su cotización queda congelada en el valor utilizado para la compensación (ver UC-15), sin nuevas actualizaciones posteriores a la baja.
...
- **Actor:** Administrador.
- **Descripción:** Un administrador dispara manualmente, fuera del ciclo semanal automático, el mismo cálculo algorítmico de cotización que ejecuta el sistema cada semana (UC-06). Está pensado para necesidades operativas — por ejemplo, validar una corrección o resolver una incidencia — nunca para fijar un valor de cotización a mano. **El disparo manual recalcula la totalidad de los jugadores activos del catálogo, exactamente el mismo alcance que el ciclo semanal automático: no existe una opción para seleccionar un jugador puntual y recalcular sólo ése de forma aislada.** (Confirmado por el dueño del producto: "se recalculan todos los jugadores del sistema de forma semanal automática, y cada vez que el admin lo haga de forma manual, hace lo mismo".)
...
- **Flujo principal:**
  ...
  2. El sistema ejecuta exactamente el mismo cálculo algorítmico que utiliza el ciclo semanal automático (UC-06), sobre todos los jugadores activos del catálogo, tomando el rendimiento disponible en ese momento.
  3. El sistema persiste la nueva cotización resultante como la vigente para cada uno de esos jugadores, conservando la anterior en el historial de cada uno, igual que en el ciclo semanal.
  4. El sistema deja registro en el log de auditoría de quién disparó el recálculo, cuándo, y los valores antes/después de cada jugador activo.
- **Flujos alternativos:**
  - Un actor sin rol ADMIN intenta disparar el recálculo: el sistema rechaza la operación.
- **Criterios de aceptación:**
  - Dado un administrador autenticado, cuando dispara el recálculo manual, entonces el sistema ejecuta el mismo algoritmo determinístico que el job semanal sobre todos los jugadores activos del catálogo y persiste la nueva cotización vigente de cada uno, conservando el historial.
  - El disparo manual no permite elegir un jugador o subconjunto de jugadores a recalcular: siempre opera sobre la totalidad de jugadores activos, igual que UC-06.
  ...
  - Toda ejecución del disparo manual queda registrada en el log de auditoría, con quién la ejecutó, cuándo, y los valores de cotización antes/después de cada jugador activo.
  - El disparo manual no reemplaza ni altera la periodicidad del ciclo semanal automático (UC-06): ambos coexisten como dos formas distintas de invocar el mismo cálculo, con idéntico alcance.
...
- **Flujo principal:**
  ...
  2. Si existen ofertas P2P abiertas (ver UC-17) con tokens de ese jugador, el sistema las cancela y reintegra la cantidad reservada de cada una a la tenencia de su vendedor correspondiente, antes de continuar.
  3. El sistema identifica, para cada usuario, la cantidad de tokens que posee de ese jugador (incluida la recién reintegrada del paso anterior), y toma la cotización que el jugador tenía el día anterior a la baja como cotización de referencia para la compensación.
  4. El sistema retira de los portfolios de los usuarios los tokens que poseían de ese jugador.
  5. El sistema acredita a cada uno de esos usuarios, como compensación, saldo virtual equivalente a la cantidad de tokens retirada multiplicada por la cotización de referencia (la del día anterior a la baja), y registra el movimiento en su historial (ver UC-12), identificado como compensación por baja y no como una venta ordinaria.
  6. El sistema retira del mercado, de forma permanente, la totalidad de los tokens emitidos del jugador: esos tokens no vuelven a estar disponibles para compra por ningún usuario.
  7. El sistema marca al jugador como inactivo y registra la operación de baja de forma trazable.
- **Flujos alternativos:**
...
- **Criterios de aceptación:**
  ...
  - Dado un jugador con una o más ofertas P2P abiertas (ver UC-17) al momento de la baja, esas ofertas se cancelan y la cantidad que tenían reservada se reintegra a la tenencia del vendedor correspondiente antes de calcular su compensación, de forma que ningún vendedor pierde tokens por tenerlos publicados en una oferta al momento de la baja.
...
- **Descripción:** Un usuario vendedor ofrece una cantidad de sus tokens de un jugador para la venta directa entre usuarios. Es una **oferta abierta**: cualquier usuario interesado con saldo suficiente puede comprarla, en todo o en parte, sin que el vendedor la dirija de antemano a un comprador específico. La operación se ejecuta automáticamente en el momento en que un comprador interesado decide comprar, sin necesidad de que el vendedor confirme esa compra puntual ni de que el comprador realice ningún paso adicional más allá de decidir comprar. El precio es siempre la cotización oficial vigente del jugador en el momento de la ejecución — el vendedor publica una cantidad de tokens, nunca un precio. **Al publicarse, la cantidad ofrecida queda reservada:** deja de estar disponible para el propio vendedor (no puede venderla al sistema vía UC-10 ni publicarla en otra oferta) hasta que la oferta se venda (total o parcialmente), el vendedor la cancele, o el jugador sea dado de baja.
- **Precondiciones:** El usuario vendedor está autenticado y posee al menos la cantidad de tokens del jugador que publica para la venta (tenencia no reservada en otra oferta). En el momento en que un comprador decide comprar, éste tiene saldo virtual suficiente para pagar la cantidad que quiere adquirir a la cotización oficial vigente.
- **Flujo principal:**
  1. El vendedor publica una cantidad de tokens de un jugador que posee, para la venta directa a cualquier usuario interesado. Esa cantidad queda **reservada** de inmediato: ya no forma parte de la tenencia que el vendedor puede vender al sistema o publicar en otra oferta.
  2. Un usuario comprador interesado decide comprar una cantidad de esa oferta (hasta el total publicado).
  3. El sistema valida que el comprador tenga saldo virtual suficiente a la cotización oficial vigente del jugador, y que la cantidad solicitada siga disponible (reservada y no vendida aún) en la oferta.
  4. El sistema ejecuta, en una única operación atómica, los cuatro movimientos: débito de los tokens reservados del vendedor, crédito del saldo virtual correspondiente al vendedor, débito del saldo virtual del comprador y crédito de los tokens al comprador.
  ...
  6. En cualquier momento antes de que la totalidad de la oferta se venda, el vendedor puede **cancelarla**: la cantidad remanente aún no vendida deja de estar reservada y vuelve a estar disponible como tenencia ordinaria del vendedor (para venderla al sistema, publicarla en una nueva oferta, etc.), sin que se registre ninguna venta sobre esa cantidad cancelada.
- **Flujos alternativos:**
  ...
  - El vendedor intenta publicar más tokens de los que posee como tenencia no reservada: la publicación se rechaza por completo.
  ...
  - El vendedor cancela la oferta antes de que se venda (total o parcialmente): la cantidad remanente vuelve a su tenencia disponible, como se describe en el flujo principal.
  - **El jugador de una oferta abierta es dado de baja (ver UC-15):** la oferta se cancela automáticamente por el sistema (el vendedor no puede seguir vendiendo esos tokens a otros usuarios); la cantidad que estaba reservada en la oferta se reintegra primero a la tenencia del vendedor, y a continuación se le aplica el mecanismo estándar de compensación por baja (UC-15) sobre la totalidad de su tenencia de ese jugador —incluidos los tokens recién reintegrados—, a la cotización del día anterior a la baja. (Confirmado por el dueño del producto: "si el jugador es dado de baja y hay un vendedor vendiendo sus tokens, se cancela la transacción, se le devuelven los tokens al usuario correspondiente y luego son reintegrados con el valor del día anterior".)
- **Criterios de aceptación:**
  ...
  - Mientras una cantidad de tokens está publicada en una oferta, esa cantidad está reservada: el vendedor no puede venderla al sistema (UC-09/UC-10) ni publicarla en otra oferta P2P simultánea; sólo puede disponer de ella si la oferta se cancela, se vende, o el jugador es dado de baja.
  - Dado que el vendedor cancela una oferta con cantidad remanente sin vender, esa cantidad deja de estar reservada y vuelve a estar disponible como tenencia ordinaria del vendedor, sin que se registre ninguna venta.
  - Dado que el jugador de una oferta abierta es dado de baja, la oferta queda cancelada, la cantidad reservada se reintegra primero a la tenencia del vendedor, y luego ese vendedor recibe la compensación de UC-15 (sobre la totalidad de su tenencia de ese jugador, incluida la reintegrada) a la cotización del día anterior a la baja.
...
11. Al publicar una oferta P2P, la cantidad ofrecida queda **reservada**: deja de formar parte de la tenencia disponible del vendedor para venderla al sistema (UC-10) o publicarla en otra oferta, hasta que se venda, se cancele, o el jugador sea dado de baja. El vendedor puede **cancelar** una oferta en cualquier momento antes de que se venda por completo; la cantidad remanente vuelve a su tenencia disponible sin que se registre venta alguna. Si el jugador de una oferta abierta es dado de baja (UC-15), la oferta se cancela automáticamente, la cantidad reservada se reintegra primero a la tenencia del vendedor, y luego se le aplica la compensación estándar de la baja sobre la totalidad de su tenencia (incluida la reintegrada), a la cotización del día anterior a la baja.
...
Una revisión posterior de consistencia entre casos de uso (no una pregunta del dueño del producto, sino una contradicción detectada entre UC-06, UC-08 y UC-15) encontró que UC-06 recalculaba literalmente "todos los jugadores del catálogo", lo cual entraba en conflicto con que la cotización de un jugador dado de baja debe quedar congelada. **Corregido:** UC-06 y UC-13 ahora recalculan explícitamente sólo jugadores **activos**; un jugador dado de baja queda excluido del recálculo semanal y del disparo manual (ver UC-06).

De esa misma revisión surgieron dos preguntas nuevas, ambas ya respondidas por el dueño del producto:

- **Alcance del disparo manual (surgida de UC-13): resuelta.** El disparo manual siempre recalcula la totalidad de los jugadores activos del catálogo, exactamente el mismo alcance que el ciclo semanal automático; no existe una opción para recalcular un jugador puntual de forma aislada (ver UC-13).
- **Ciclo de vida de una oferta P2P (surgida de UC-17): resuelta.** Al publicarse, la cantidad ofrecida queda reservada (deja de estar disponible para el vendedor por otra vía) hasta que se venda, se cancele por el propio vendedor, o el jugador sea dado de baja — caso en el cual la oferta se cancela automáticamente, los tokens reservados se reintegran al vendedor, y luego se le aplica la compensación estándar de la baja (ver UC-15 y UC-17, y regla 11 de la sección 5).
